...
(define-foreign-type AVRational (c-pointer "AVRational"))
...
(define-getters fmtx ((AVFormatContext x))
  ...
  (input-format     AVInputFormat  "x->iformat")
  (filename         c-string       "x->filename" #f))

(define-getters codecpar ((AVCodecParameters cp))
  (type                   AVMediaType                          "cp->codec_type")
  (id                     AVCodecID                            "cp->codec_id")
  (tag                    int                                  "cp->codec_tag")
  ;;(*extradata           byte                                 "cp->*extradata")
  ;;(extradata-size       int                                  "cp->extradata_size")
  (format*                int                                  "cp->format")
  (bit-rate               integer64                            "cp->bit_rate")
  (bits-per-coded-sample  int                                  "cp->bits_per_coded_sample")
  (bits-per-raw-sample    int                                  "cp->bits_per_raw_sample")
  (profile                int                                  "cp->profile")
  (level                  int                                  "cp->level")
  (width                  int                                  "cp->width")
  (height                 int                                  "cp->height")
  (sample-aspect-ratio    AVRational                           "cp->sample_aspect_ratio")
  ;;(field-order            enum-AVFieldOrder                  "cp->field_order")
  ;;(color-range            enum-AVColorRange                  "cp->color_range")
  ;;(color-primaries        enum-AVColorPrimaries              "cp->color_primaries")
  ;;(color-trc              enum-AVColorTransferCharacteristic "cp->color_trc")
  ;;(color-space            enum-AVColorSpace                  "cp->color_space")
  ;;(chroma-location        enum-AVChromaLocation              "cp->chroma_location")
  (video-delay            int                                  "cp->video_delay")
  (channel-layout         unsigned-integer64                   "cp->channel_layout")
  (channels               int                                  "cp->channels")
  (sample-rate            int                                  "cp->sample_rate")
  (block-align            int                                  "cp->block_align")
  (frame-size             int                                  "cp->frame_size")
  (initial-padding        int                                  "cp->initial_padding")
  (trailing-padding       int                                  "cp->trailing_padding")
  (seek-preroll           int                                  "cp->seek_preroll"))

(define-getters frame ((AVFrame x))
  ;;(data*                     c-pointer                                                 "x->data")
  ;;(linesize*                 c-pointer                                "x->linesize")
  ;;(**extended-data                uint8_t                            "x->**extended_data")
  (width                          int                                "x->width")
  (height                         int                                "x->height")
  (sample-count                   int                                "x->nb_samples")
  (format*                        int                                "x->format")
  (key-frame                      int                                "x->key_frame")
  (pict-type                      AVPictureType                      "x->pict_type")
  (sample-aspect-ratio            AVRational                         "x->sample_aspect_ratio")
  (pts                            integer64                          "x->pts")
  (pkt-dts                        integer64                          "x->pkt_dts")
  (coded-picture-number           int                                "x->coded_picture_number")
  (display-picture-number         int                                "x->display_picture_number")
  (quality                        int                                "x->quality")
  ;;(*opaque                        void                               "x->*opaque")
  (repeat-pict                    int                                "x->repeat_pict")
  (interlaced-frame               int                                "x->interlaced_frame")
  (top-field-first                int                                "x->top_field_first")
  (palette-has-changed            int                                "x->palette_has_changed")
  (reordered-opaque               integer64                          "x->reordered_opaque")
  (sample-rate                    int                                "x->sample_rate")
  (channel-layout                 unsigned-integer64                 "x->channel_layout")
  ;;(*buf[AV_NUM_DATA_POINTERS]     AVBufferRef                        "x->*buf"[AV_NUM_DATA_POINTERS])
  ;;(**extended-buf                 AVBufferRef                        "x->**extended_buf")
  ;;(nb-extended-buf                int                                "x->nb_extended_buf")
  ;;(**side-data                    AVFrameSideData                    "x->**side_data")
  ;;(nb-side-data                   int                                "x->nb_side_data")
  (flags                          int                                "x->flags")
  ;;(color-range                    enum-AVColorRange                  "x->color_range")
  ;;(color-primaries                enum-AVColorPrimaries              "x->color_primaries")
  ;;(color-trc                      enum-AVColorTransferCharacteristic "x->color_trc")
  ;;(colorspace                     enum-AVColorSpace                  "x->colorspace")
  ;;(chroma-location                enum-AVChromaLocation              "x->chroma_location")
  (best-effort-timestamp          integer64                          "x->best_effort_timestamp")
  (pkt-pos                        integer64                          "x->pkt_pos")
  (pkt-duration                   integer64                          "x->pkt_duration")
  ;;(*metadata                      AVDictionary                       "x->*metadata")
  (decode-error-flags             int                                "x->decode_error_flags")
  (channels                       int                                "x->channels")
  (pkt-size                       int                                "x->pkt_size")
  ;;(*hw-frames-ctx                 AVBufferRef                        "x->*hw_frames_ctx")
  ;;(*opaque-ref                    AVBufferRef                        "x->*opaque_ref")
  (crop-top                       size_t                             "x->crop_top")
  (crop-bottom                    size_t                             "x->crop_bottom")
  (crop-left                      size_t                             "x->crop_left")
  (crop-right                     size_t                             "x->crop_right"))

(define-getters codecx ((AVCodecContext x))
  (width                  int            "x->width")
  (height                 int            "x->height")
  (pix-fmt                AVPixelFormat  "x->pix_fmt")
  (bit-rate               integer64      "x->bit_rate")
  (time-base              AVRational     "x->time_base")
  (framerate              AVRational     "x->framerate")
  (slices                 int            "x->slices")
  (sample-rate            int            "x->sample_rate")
  (channels               int            "x->channels")
  (sample-fmt             AVSampleFormat "x->sample_fmt")
  (frame-size             int            "x->frame_size")
  (frame-number           int            "x->frame_number")
  (channel-layout         unsigned-integer64 "x->channel_layout")
  (request-channel-layout unsigned-integer64 "x->request_channel_layout")
  (qcompress              float          "x->qcompress")
  (qblur                  float          "x->qblur")
  (qmin                   int            "x->qmin")
  (qmax                   int            "x->qmax")
  (max-qdiff              int            "x->max_qdiff")
  (qcompress              float          "x->qcompress")
  (qblur                  float          "x->qblur")
  (qmin                   int            "x->qmin")
  (qmax                   int            "x->qmax")
  ...
  (qmax                   int            "x->qmax")
  (max-qdiff              int            "x->max_qdiff")
  (rc-buffer-size         int            "x->rc_buffer_size")
  (rc-max-rate            integer64      "x->rc_max_rate")
  (rc-min-rate            integer64      "x->rc_min_rate")
  (stats-out              c-string       "x->stats_out")
  (stats-in               c-string       "x->stats_in"))

(define-getters codec ((AVCodec x))
  (name      c-string  "x->name")
  (long-name c-string  "x->long_name")
  (type      AVMediaType "x->type")
  (id*       int         "x->id")
  (id        AVCodecID   "x->id"))


(define-getters filter ((AVFilter x))
  (name        c-string  "x->name")
  (description c-string  "x->description"))

(define-getters filterx ((AVFilterContext x))
  (name        c-string  "x->name")
  (filter      AVFilter  "x->filter"))
;; ^ TODO: inputs outputs

(define-getters filterg ((AVFilterGraph x))
  (filter-count    unsigned-int  "x->nb_filters")
  (sink-links-count int "x->sink_links_count"))
;; ^ TODO: filters

(define-getters input-format ((AVInputFormat x))
  (name        c-string  "x->name"))

(define-getters output-format ((AVOutputFormat x))
  (name        c-string  "x->name"))
...
(define-getters stream ((AVStream x))
  (index            int               "x->index")
  (time-base        AVRational       "x->time_base")
  (avg-frame-rate   AVRational       "x->avg_frame_rate")
  (id               int               "x->id")
  (codecpar         AVCodecParameters "x->codecpar"))
